...
import type { ParsedRecipe, ParsedRecipeResult } from "@recipes/shared";
import { hasVariants } from "@recipes/shared";
import { generateRecipe, createRecipe, createRecipeWithVariants } from "../api/client";
...
export function RecipeGenerator({ onRecipeCreated }: Props) {
  ...
  const [generatedRecipe, setGeneratedRecipe] = useState<ParsedRecipeResult | null>(
    null
  );
  ...
  async function handleSave() {
    ...
    try {
      if (hasVariants(generatedRecipe)) {
        // Multiple portion variants - auto-save all
                ...
                <div class="recipe-generator-preview">
                  {hasVariants(generatedRecipe) ? (
                    <>
                      ...
                      <p class="recipe-generator-variants-notice">
                        Multiple portion sizes detected:{" "}
                        {generatedRecipe.variants
                          .map((v) => `${v.servings} servings`)
                          .join(", ")}
                      </p>
